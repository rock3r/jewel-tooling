...
object AgentRuntime {
  ...
  fun start(argument: String, instrumentation: Instrumentation) {
    ...
    val nonce = checkNotNull(values["nonce"]).also { require(it.matches(Regex("[0-9a-f]{64}"))) }
    val name = checkNotNull(values["name"]).also { require(it.isNotBlank() && it.length <= 256) }
    ...
    try {
      ...
      InspectionFiles.write(
        ...
        mapOf(
          ...
          "started" to process.info().startInstant().orElseThrow().toEpochMilli().toString(),
...
private class AgentHost(name: String, private val instrumentation: Instrumentation) :
  ...
  fun install() {
    ...
    lease.schedule({ if (!connected.get()) close() }, CONNECTION_LEASE_SECONDS, TimeUnit.SECONDS)
  }
  ...
  private fun runtimeLoaded(loader: ClassLoader?, module: Module?): Int =
    synchronized(runtimeLock) {
      ...
      if (module != null && module.isNamed && !module.canRead(TraceBridge::class.java.module)) {
  ...
  private fun available(runtime: Int) =
    synchronized(runtimeLock) {
      if (
        runtime == 1 && discovered == 1 && runtimeStatus.get().state == LiveRuntimeState.NO_RUNTIME
      )
  ...
  private fun unavailable(state: LiveRuntimeState) {
    runtimeStatus.updateAndGet { previous ->
      when {
        ...
        previous.state !in setOf(LiveRuntimeState.NO_RUNTIME, LiveRuntimeState.READY) -> previous
  ...
  override fun enabled(runtime: Int): Boolean =
    runtime == 1 &&
      runtimeStatus.get().state == LiveRuntimeState.READY &&
  ...
  private inner class Session : LiveRecordingSession {
    ...
    @Suppress(
      ...
      "ThrowsCount",
    ) // Transfer failures across the EDT boundary.
    private fun onEdt(action: () -> Recording?): Recording? {
      ...
      return try {
        ...
        if (failure.cause is IllegalStateException)
          throw IllegalStateException("Capture is unavailable", failure)
        throw IOException("Capture command failed", failure)
      } catch (failure: TimeoutException) {
        abort()
        throw IOException("Composition thread timed out", failure)
      } catch (failure: InterruptedException) {
        abort()
        Thread.currentThread().interrupt()
        throw IOException("Capture command interrupted", failure)
      }
    }
