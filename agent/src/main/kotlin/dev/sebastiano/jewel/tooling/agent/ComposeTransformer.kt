...
internal class ComposeTransformer(
    ...
    private fun checkAbi(node: ClassNode) {
        ...
        for (method in
            node.methods.filter {
                it.name.startsWith("traceEvent") || it.name.startsWith("isTrace")
            }) {
    ...
    private fun checkLegacyStart(method: MethodNode) {
        ...
        require(
            calls.all {
                (it.owner == COMPOSER && it.name == "traceEventStart" && it.desc == START) ||
                    (it.owner == "kotlin/jvm/internal/Intrinsics" &&
                        it.name == "checkNotNullParameter")
            }
        )
        val allowed =
            setOf(
    ...
    companion object {
        ...
        private val REQUIRED =
            mapOf(
                "isTraceInProgress" to "()Z",
                "traceEventStart" to START,
                "traceEventEnd" to "()V",
            )
    }
}
