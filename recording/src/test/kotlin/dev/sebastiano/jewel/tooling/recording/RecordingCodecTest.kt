...
class RecordingCodecTest {
    ...
    fun legacyFinalAndLiveSnapshotsHaveSeparateFileRules() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            live.copy(schemaVersion = 1).validate()
        }
        ...
        try {
            assertThrows(RecordingFormatException::class.java) {
                RecordingFiles.writeNew(path, live)
            }
    ...
    @Test
    fun liveSnapshotSummariesRoundTripWithoutEvents() {
        val site = sample().sites.single()
        val live =
            sample()
                .copy(
                    ...
                    events = emptyList(),
                    summaries = listOf(SiteSummary(site, 3, 24, mapOf(1L to 3))),
                )
        assertEquals(live, read(RecordingCodec.write(live).toString(Charsets.UTF_8)))
        assertEquals(3, live.completedExecutions())
        assertTrue(json().contains("\"events\":[") && !json().contains("siteSummaries"))
    }
    ...
    fun duplicateIdsAndExcessEventsAreRejected() {
        ...
        val many =
            sample().copy(events = List(RecordingLimits.EVENTS + 1) { sample().events.single() })
