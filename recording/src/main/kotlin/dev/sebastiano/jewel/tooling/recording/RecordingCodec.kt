...
object RecordingCodec {
    ...
    private const val JSON_TOKENS = 2_500_000L
    ...
    fun write(recording: Recording, checkCanceled: () -> Unit = {}): ByteArray {
        ...
        factory.createGenerator(output).use { writer ->
            ...
            writer.writeEndArray()
            val summaries = recording.summaries
            if (summaries != null) {
                writer.writeArrayFieldStart("siteSummaries")
                for (summary in summaries) {
                    checkCanceled()
                    writer.writeStartObject()
                    writer.writeNumberField("siteId", summary.site.id)
                    writer.writeNumberField("executions", summary.executions)
                    writer.writeNumberField("totalNs", summary.totalNs)
                    writer.writeObjectFieldStart("threads")
                    for ((threadId, count) in summary.threads) {
                        writer.writeNumberField(threadId.toString(), count)
                    }
                    writer.writeEndObject()
                    writer.writeEndObject()
                }
    ...
    private class Reader(private val parser: JsonParser, private val checkCanceled: () -> Unit) {
        ...
        fun recording(): Recording {
            ...
            var summaryRecords: List<SummaryRecord>? = null
            fields(
                expected =
                    setOf(
                        "schemaVersion",
                ...
                when (name) {
                    ...
                    "capabilities" ->
                        capabilities = array(RecordingLimits.CAPABILITIES.size) { string() }
                    ...
                    "siteSummaries" ->
                        summaryRecords = array(RecordingLimits.SITES) { summaryRecord() }
                }
            }
            if (
            ...
            val resolvedSites = checkNotNull(sites)
            return Recording(
                session,
                checkNotNull(target),
                duration,
                ...
                checkNotNull(events),
                summaryRecords?.let { records ->
                    val siteMap = resolvedSites.associateBy { it.id }
                    java.util.List.copyOf(
                        records.map { record ->
                            SiteSummary(
                                checkNotNull(siteMap[record.siteId]),
                                record.executions,
                                record.totalNs,
                                record.threads,
                            )
        ...
        private fun summaryRecord(): SummaryRecord {
            var siteId = 0
            var executions = 0
            var totalNs = 0L
            var threads: Map<Long, Int>? = null
            fields(setOf("siteId", "executions", "totalNs", "threads")) {
                when (it) {
                    "siteId" -> siteId = integer()
                    "executions" -> executions = integer()
                    "totalNs" -> totalNs = long()
                    "threads" -> threads = threadCounts()
                }
            }
            return SummaryRecord(siteId, executions, totalNs, checkNotNull(threads))
        }

        private fun threadCounts(): Map<Long, Int> {
            requireFormat(parser.currentToken() == JsonToken.START_OBJECT)
            val values = LinkedHashMap<Long, Int>()
            while (parser.nextToken() != JsonToken.END_OBJECT) {
                checkCanceled()
                requireFormat(parser.currentToken() == JsonToken.FIELD_NAME)
                val parsed = parser.currentName().toLongOrNull()
                requireFormat(parsed != null && parsed > 0 && values.size < RecordingLimits.THREADS)
                val threadId = checkNotNull(parsed)
                requireFormat(parser.nextToken() != null)
                requireFormat(values.put(threadId, integer()) == null)
            }
            requireFormat(values.isNotEmpty())
        ...
        private fun fields(
            ...
            requireFormat(parser.currentToken() == JsonToken.START_OBJECT)
            val known = expected + optional
            val seen = HashSet<String>()
            while (parser.nextToken() != JsonToken.END_OBJECT) {
                checkCanceled()
                requireFormat(parser.currentToken() == JsonToken.FIELD_NAME)
                val name = parser.currentName()
                requireFormat(name in known && seen.add(name))
                requireFormat(parser.nextToken() != null)
                read(name)
            }
            requireFormat(expected.all { it in seen } && seen.all { it in known })
        }
