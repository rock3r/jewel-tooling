...
class RuntimeServerTest {
        ...
        endpoint { _, arguments ->
            ...
            .use { server ->
                connected(server) { client, _ ->
                    ...
                    val result =
                        client.callTool(
                            CallToolRequest(CallToolRequestParams(name = "jewel_status"))
                        )
                    assertEquals(
                        "result",
        ...
        endpoint { _, _ -> CompletableFuture.completedFuture(RESULT) }
            .use { server ->
                for (method in listOf("POST", "GET", "DELETE")) {
                    assertEquals(
                        401,
                        request(server, method, token = null, body = "x".repeat(70_000)),
                    )
    ...
    fun endpointCredentialsDoNotAuthorizeAnotherProject() {
        endpoint { _, _ -> CompletableFuture.completedFuture(RESULT) }
            .use { first ->
                RuntimeServer.start(
                        mapOf("token" to TOKEN.reversed(), "pluginVersion" to "test"),
                        TOOLS,
                    ) { _, _ ->
                        CompletableFuture.completedFuture(RESULT)
                    }
    ...
    companion object {
        ...
        private const val TOOLS =
            """[{"name":"jewel_status","description":"Status",""" +
                """"inputSchema":{"type":"object","properties":{}},""" +
                """"outputSchema":{"type":"object","properties":{"schemaVersion":{"type":"integer"},""" +
                """"payload":{"type":"object"}},"required":["schemaVersion","payload"]},""" +
