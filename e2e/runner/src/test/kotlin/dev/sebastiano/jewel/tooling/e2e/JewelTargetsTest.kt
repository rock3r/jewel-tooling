            ...
            Starter.newContext(
                    ...
                    TestCase(
                        IdeaUltimateProductInit()
                            ...
                            .copy(
                                buildType = "release",
                                buildNumber = System.getProperty("jewel.test.ideBuild"),
                            )
                            .let { info ->
                                ...
                                if (local == null) info
                                    ...
                                    info.copy(
                                        getInstaller = { ExistingIdeInstaller(Path.of(local)) }
                                    )
                            },
                ...
                .apply {
                    ...
                    installer.installPluginFromPath(
                        Path.of(System.getProperty("jewel.test.productionZip"))
                    )
                    installer.installPluginFromPath(
                        Path.of(System.getProperty("jewel.test.driverZip"))
                    )
                    if (target == "ijpl")
                        installer.installPluginFromPath(
                ...
                .applyVMOptionsPatch {
                    ...
                    addSystemProperty(
                        "jewel.test.targetHome",
                        temporary.resolve("target-ide").toString(),
                    )
                    addSystemProperty(
                        "jewel.test.driverZip",
                        System.getProperty("jewel.test.driverZip"),
                    )
                    addSystemProperty(
                        "jewel.test.fixtureZip",
                        repository
                            .resolve("fixtures/ijpl/build/distributions/ijpl-fixture.zip")
                            .toString(),
                    )
                    ...
                    addSystemProperty(
                        ...
                        repository
                            .resolve("fixtures/standalone/build/capture/recording.json")
                            .toString(),
                    )
                    addSystemProperty(
        ...
        context.runIdeWithDriver().useDriverAndCloseIde {
            ...
            try {
                invokeGlobalBackendAction(
                    if (oneClick) "JewelTooling.RunLaunchScenario"
                    else "JewelTooling.RunEditorScenario",
                    singleProject(),
                    now = false,
                )
                ...
                check(
                    Files.readString(output.resolve("recording-lifecycle.txt")).startsWith("PASS:")
                )
