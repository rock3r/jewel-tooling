...
internal class LiveScenario(private val project: Project) {
  ...
  suspend fun run(robot: RobotDriver, output: Path) {
    ...
    await {
      val observed = edt {
        status(tool.component) to
          descendants(tool.component).filterIsInstance<javax.swing.JTextArea>().joinToString("\n") {
            it.text
          }
      }
      Files.writeString(
    ...
    await {
      edt {
        val texts = descendants(tool.component).filterIsInstance<javax.swing.text.JTextComponent>()
        Files.writeString(
          output.resolve("live-export-state.txt"),
          texts.joinToString("\n") { it.text },
        )
        texts.any { it.text.startsWith("The recording was exported.") }
      }
    }
    Files.writeString(output.resolve("live-stage.txt"), "start another capture")
    click("Start Capture")
    ...
    Files.writeString(
      ...
      "PASS: live counts increased twice before stop; final export matches; " +
        "new capture active for unload; executions=$stopped",
    )
  }
  ...
  private suspend fun interact(ijpl: Boolean, robot: RobotDriver, output: Path, sequence: Int) {
    if (ijpl) {
      edt {
        checkNotNull(ToolWindowManager.getInstance(project).getToolWindow("Jewel Fixture")).show()
      }
      val automator = ComposeAutomator.inProcess(robotDriver = robot)
      Files.writeString(output.resolve("live-stage.txt"), "wait for add-item node ($sequence)")
  ...
  private suspend fun connectThroughDialog(component: Component, endpoint: String) {
    ...
    edt(modality) {
      descendants(dialog).filterIsInstance<javax.swing.JPasswordField>().single().text = endpoint
  ...
  private suspend fun click(text: String) {
    ...
    val window = edt { checkNotNull(javax.swing.SwingUtilities.getWindowAncestor(tool.component)) }
