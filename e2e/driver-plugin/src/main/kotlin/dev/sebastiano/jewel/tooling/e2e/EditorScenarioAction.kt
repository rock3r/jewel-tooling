...
class EditorScenarioAction : AnAction() {
            ...
            try {
                runBlocking {
                    withTimeout(SCENARIO_TIMEOUT_MS) {
                        ...
                        val expected =
                            System.getProperty("jewel.test.expected", "stable,unstable").split(",")
                        if (System.getProperty("jewel.test.target") != null) {
                            Files.writeString(
                                ...
                                runReadActionBlocking {
                                    "file=$path; canonical=${path.toRealPath()}\n" +
                                        ModuleManager.getInstance(project).modules.joinToString(
                                            ...
                                            "${it.name}: content=${roots.contentRoots.toList()}; " +
                                                "sources=${roots.sourceRoots.toList()}"
                                        }
                                },
                            )
                            val evidence = runReadActionBlocking {
                                ...
                                val module =
                                    requireNotNull(ModuleUtilCore.findModuleForPsiElement(psi))
                        ...
                        if (java.lang.Boolean.getBoolean("jewel.test.mcp")) {
                            ...
                            Files.writeString(
                                ...
                                "PASS: MCP setup and static analysis",
                            )
                            return@withTimeout
                        }
                        ...
                        val frame = edt {
                            requireNotNull(WindowManager.getInstance().getFrame(project))
                        }
                        ...
                        Files.writeString(
                            output.resolve("diagnostics.txt"),
                            errors.joinToString("\n"),
                        )
                        check(errors.isEmpty()) { "Editor errors: $errors" }
                        val region = edt {
                            Rectangle(editor.component.locationOnScreen, editor.component.size)
                                .apply { height = minOf(height, EDITOR_CAPTURE_HEIGHT) }
                        }
                        val transform = edt {
                            editor.contentComponent.graphicsConfiguration.defaultTransform
                        }
                ...
                "scaleY":${transform.scaleY}}""",
                        ...
                        check(
                            explanations.any { it.contains("Function types are treated as stable") }
                        )
                        ...
                        await(stage) {
                            ...
                            Files.writeString(
                                output.resolve("edited-inlays.txt"),
                                actual.joinToString(","),
                            )
                            Files.writeString(
                                ...
                                edt {
                                    ...
                                    "active=${ApplicationManager.getApplication().isActive}; " +
                                        "focus=${focus.focusOwner?.javaClass?.name}; " +
                                        "window=${focus.activeWindow?.javaClass?.name}; " +
                ...
                "scaleY":${transform.scaleY}}""",
                        ...
                        if (System.getProperty("jewel.test.target") != null) {
                            ...
                            recording.verifyUnloadReload(
                                output.resolve("live-recording.json"),
                                output,
                            )
                        ...
                        Files.writeString(
                            output.resolve("result.txt"),
                            "PASS: rendered labels, explanations, edit invalidation, " +
                                "provider toggle, Spectre device-scale capture",
                        )
                    }
                }
    ...
    internal fun importGradle(project: Project) {
        ...
        edt {
            ...
            if (
                settings.linkedProjectsSettings.none { it.externalProjectPath == project.basePath }
            ) {
                    ...
                    .withCallback(
                        object : ExternalProjectRefreshCallback {
                            override fun onSuccess(externalProject: DataNode<ProjectData>?) {
                                try {
                                    checkNotNull(externalProject) {
                                        "Gradle sync returned no project model"
                                    }
                            ...
                            override fun onFailure(errorMessage: String, errorDetails: String?) {
                                completed.completeExceptionally(
                                    IllegalStateException(
                                        "Gradle sync failed: $errorMessage\n$errorDetails"
                                    )
                                )
                            }
                        }
