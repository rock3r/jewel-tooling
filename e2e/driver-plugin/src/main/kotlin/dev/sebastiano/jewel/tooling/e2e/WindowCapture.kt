...
internal object WindowCapture {
  fun capture(window: Window, region: Rectangle, robot: RobotDriver): BufferedImage {
    ...
    val image = AutoScreenshotter().captureWindow(target)
  ...
  private fun titled(window: Window): TitledWindow {
    // Resolve on EDT before entering AppKit: AppKit must never synchronously call back into EDT.
    val nativeWindow =
      ...
      override var title: String?
        get() = edt {
          when (window) {
            ...
            else -> native { Foundation.toStringViaUTF8(Foundation.invoke(nativeWindow, "title")) }
          }
        }
        set(value) {
          edt {
            when (window) {
              ...
              is Dialog -> window.title = value.orEmpty()
                ...
                native {
                  Foundation.invoke(nativeWindow, "setTitle:", Foundation.nsString(value.orEmpty()))
                }
            }
          }
        }
