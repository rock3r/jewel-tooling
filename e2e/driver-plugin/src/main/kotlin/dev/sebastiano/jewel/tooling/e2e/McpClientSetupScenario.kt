...
internal object McpClientSetupScenario {
    suspend fun install(window: JDialog, output: Path) {
        ...
        val point = edt {
            val button =
                descendants(window).filterIsInstance<JButton>().single { it.text == "Install" }
            check(button.isEnabled)
            button.locationOnScreen.apply { translate(button.width / 2, button.height / 2) }
        }
        RobotDriver.synthetic(rootWindow = generateSequence<Window>(window) { it.owner }.last())
            .use { it.click(point.x, point.y) }
        await {
            edt {
                labels(window).any { it.contains("Configuration installed. Refresh the client.") }
            }
        }
    ...
    private suspend fun installSkill(window: JDialog, output: Path) {
        ...
        val receipt =
            JsonParser.parseString(
                    Files.readString(file.resolveSibling(".jewel-tooling-skill.json"))
                )
                .asJsonObject
    ...
    private suspend fun verifyLaunch(command: List<String>) {
        ...
        try {
            ...
            val reply =
                JsonParser.parseString(bytes.toString(Charsets.UTF_8).lineSequence().first())
                    .asJsonObject
