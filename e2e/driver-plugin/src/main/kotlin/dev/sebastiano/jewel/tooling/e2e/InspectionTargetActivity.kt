...
class InspectionTargetActivity : ProjectActivity {
  ...
  override suspend fun execute(project: Project) {
    ...
    withContext(Dispatchers.IO) {
      Files.writeString(output.resolve("target-ready"), ProcessHandle.current().pid().toString())
    }
    var previous = ""
    while (!project.isDisposed) {
      if (withContext(Dispatchers.IO) { Files.exists(output.resolve("target-stop")) }) {
        withContext(Dispatchers.EDT) {
      ...
      if (command.isNotEmpty() && command != previous) {
        ...
        withContext(Dispatchers.IO) { Files.writeString(output.resolve("target-clicked"), command) }
        previous = command
      }
