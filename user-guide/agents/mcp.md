...
Run only the command for your client. Restart its session or reconnect the server through its MCP controls. The commands use the documented [Codex MCP syntax](https://developers.openai.com/codex/mcp/) and [Claude Code stdio syntax](https://code.claude.com/docs/en/mcp). Their argument forms were checked against the installed CLIs.
...
Use the actual returned ID. For an extension receiver, use the parameter name `<receiver>`. The explain tool describes a parameter's resolved type in context. It does not resolve arbitrary type strings.
