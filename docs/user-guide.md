...
The Bazel E2E boundary is a model exported from a real Bazel build. It does not test a third-party Bazel importer wizard. Kotlin Multiplatform source-set models are untested in v1. Unannotated types crossing IDE module boundaries remain unknown.
...
| Hint | Meaning |
| --- | --- |
| stable | A recognized stable type, a trusted Compose stability contract, or an inferred final source class whose stored properties have stable types. |
| unstable | A standard collection or array, a vararg, or a class with a mutable or unstable stored property. |
...
The plugin does not interpret compiler stability metadata, external stability configuration files, or custom and inherited stability contracts. It leaves unannotated library types such as `Pair` and `Triple` unknown. Singleton objects and value classes, including unsigned types, are also unknown unless a recognized contract applies.
...
Analysis processes the extension receiver, then parameters in declaration order. It stops after 256 type visits for a function or a nesting depth of 12. Earlier completed hints remain; exhausted assessments say unknown and explain the budget limit. No result is retained across editing passes.
...
Install Bazelisk before preparing the Bazel fixture; its version is selected by `fixtures/ijpl/.bazelversion`. The fixture uses a small Kotlin Bazel action and exports `JavaInfo`. It does not depend on an IntelliJ source checkout.
