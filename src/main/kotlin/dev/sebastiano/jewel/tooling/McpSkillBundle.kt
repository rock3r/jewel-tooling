...
internal class McpSkillBundle(val text: String) {
    ...
    companion object {
        ...
        fun bundled(): McpSkillBundle =
            McpSkillBundle(
                requireNotNull(
                        McpSkillBundle::class.java.getResourceAsStream("/skills/$ID/SKILL.md")
                    )
    ...
    fun destination(client: McpClient): Path {
        val root =
            when (client) {
                ...
                McpClient.CLAUDE ->
                    env("CLAUDE_CONFIG_DIR", home.resolve(".claude")).resolve("skills")
                McpClient.PI ->
                    env("PI_CODING_AGENT_DIR", home.resolve(".pi/agent")).resolve("skills")
