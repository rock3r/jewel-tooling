...
constructor(
                        ...
                        withContext(Dispatchers.EDT) {
                            if (canShow(editor, stamp))
                                HintManager.getInstance()
                                    .showInformationHint(
                                        editor,
                                        JewelToolingBundle.message("details.failed"),
                                    )
                        }
                        return@launch
                    }
                withContext(Dispatchers.EDT) {
                    ...
                    if (report == null)
                        HintManager.getInstance()
                            .showInformationHint(
                                editor,
                                JewelToolingBundle.message("details.unavailable"),
                            )
    ...
    internal fun navigate(
        ...
        val alive =
            !disposed && !project.isDisposed && !editor.isDisposed && popup?.isDisposed == false
        ...
        when (status) {
            ...
            NavigationStatus.INVALID ->
                HintManager.getInstance()
                    .showInformationHint(
                        editor,
                        JewelToolingBundle.message("details.navigate.stale"),
                    )
