...
constructor(
    ...
    private var pendingSite: Int? = null
        ...
        job =
            scope.launch(ioDispatcher + modality) {
                try {
                    ...
                    withContext(Dispatchers.EDT) {
                        if (current(requestGeneration))
                            render(disconnectedState("live.connection.failed"))
                    }
                } finally {
                    client.close()
                    queue.close()
                    withContext(Dispatchers.EDT) {
                        if (current(requestGeneration))
                            project
                                .getServiceIfCreated(InspectionLaunchService::class.java)
                                ?.connectionEnded()
                    }
                }
            }
    ...
    private suspend fun receive(
        ...
        while (true) {
                    ...
                    withContext(Dispatchers.EDT) {
                        if (current(requestGeneration))
                            render(
                                state.copy(
                                    phase = LivePhase.READY,
                                    message = "live.command.unavailable",
                                )
                            )
                    }
                    continue
            ...
            if (snapshot != null) {
                ...
                logTerminalCapture(snapshot, active)
                renderSnapshot(snapshot, runtime, active, requestGeneration)
            }
        }
    }

    private fun logTerminalCapture(snapshot: Recording, active: Boolean) {
        if (
            snapshot.status != CaptureStatus.FAILED &&
                snapshot.status != CaptureStatus.TRUNCATED &&
                (active || snapshot.completedExecutions() > 0)
        )
            return
        val fidelity = snapshot.fidelity
        LOG.warn(
            "Live inspection capture ${snapshot.status}: session=${snapshot.sessionId} " +
                "reason=${snapshot.stopReason} executions=${snapshot.completedExecutions()} " +
                "durationNs=${snapshot.durationNs} abandoned=${fidelity.abandonedStarts} " +
                "discarded=${fidelity.discardedPairs} unmatched=${fidelity.unmatchedEnds} " +
                "rejected=${fidelity.rejectedStarts}"
        )
    }
                ...
                render(
                    state.copy(
                        ...
                        phase =
                            ...
                            else if (runtime.state == LiveRuntimeState.NO_RUNTIME)
                                LivePhase.WAITING_RUNTIME
    ...
    private fun allowDiscard(
        promptKey: String = "live.discard.prompt",
        confirmKey: String = "live.discard.confirm",
    ): Boolean {
        val recording = state.data?.recording
        if (
            recording == null ||
                recording.completedExecutions() == 0 ||
                recording.sessionId == exportedSession
        )
            return true
        return Messages.showYesNoDialog(
            ...
            text(confirmKey),
            text(
                if (recording.status == CaptureStatus.ACTIVE) "live.discard.keep"
                else "live.discard.cancel"
            ),
            Messages.getQuestionIcon(),
        ) == Messages.YES
    }

    fun chooseRecording(requestScope: CoroutineScope = scope) {
        if (disposed || project.isDisposed) return
        if (!importEnabled()) {
            Messages.showErrorDialog(project, text("live.open.busy"), text("recording.open.title"))
            return
        }
        val descriptor =
            FileChooserDescriptorFactory.createSingleFileDescriptor("json")
                .withFileFilter { it.isInLocalFileSystem }
                .withTitle(text("recording.open.title"))
        FileChooser.chooseFile(descriptor, project, null) { file ->
            if (!file.isInLocalFileSystem) {
                Messages.showErrorDialog(
                    project,
                    text("recording.error.read"),
                    text("recording.open.title"),
                )
                return@chooseFile
            }
            if (!allowDiscard("live.discard.open.prompt", "live.discard.open.confirm"))
                return@chooseFile
            openRecording(file.toNioPath(), requestScope)
        }
    }

    fun openRecording(path: Path, requestScope: CoroutineScope = scope) {
        if (disposed || project.isDisposed) return
        importJob?.cancel()
        if (state.connected || connection != null || state.launching) disconnect()
        val requestGeneration = generation
        val modality = ModalityState.current().asContextElement()
        importJob =
            requestScope.launch(ioDispatcher + modality) {
                ...
                withContext(Dispatchers.EDT) {
                    ...
                    if (!current(requestGeneration)) return@withContext
                    exportedSession = data.recording.sessionId
                    render(
                        LiveInspectionState(
                            data = data,
                            message = "live.opened",
    ...
    fun closeRecording() {
        if (disposed || state.data == null || !recordingActionsEnabled()) return
        if (!allowDiscard("live.discard.close.prompt", "live.discard.close.confirm")) return
        exportedSession = null
        if (state.connected) {
            val nextPhase =
                if (state.phase == LivePhase.STOPPED && state.runtimeReady) LivePhase.READY
                else state.phase
            val nextMessage =
                when (nextPhase) {
                    LivePhase.READY -> "live.ready"
                    LivePhase.WAITING_RUNTIME -> "live.runtime.waiting"
                    else -> state.message
                }
            render(state.copy(data = null, phase = nextPhase, message = nextMessage))
        } else {
            render(
                LiveInspectionState(
                    supportInstalled = state.supportInstalled,
                    message = "live.welcome",
                )
            )
        }
    }

    private fun importEnabled(): Boolean =
        !state.launching && state.phase != LivePhase.CONNECTING && state.phase != LivePhase.BUSY

    private fun recordingActionsEnabled(): Boolean =
        importEnabled() && state.phase != LivePhase.CAPTURING
    ...
    private fun disconnectedState(message: String): LiveInspectionState {
        ...
        if (recording?.status == CaptureStatus.ACTIVE) {
            LOG.warn(
                "Live inspection connection ended during capture: session=${recording.sessionId} " +
                    "executions=${recording.completedExecutions()} durationNs=${recording.durationNs}"
            )
        }
        return state.copy(
            ...
            message =
                when {
                    recording?.status == CaptureStatus.ACTIVE -> "live.incomplete"
                    recording?.status == CaptureStatus.TRUNCATED -> "live.finished"
                    else -> message
                },
        )
    }
    ...
    fun export(path: Path, recording: Recording) {
        ...
        scope.launch(ioDispatcher + modality) {
            ...
            withContext(Dispatchers.EDT) {
                if (!disposed && !project.isDisposed) {
                    if (failure != null)
                        Messages.showErrorDialog(project, text(failure), text("live.export"))
                    else {
                        if (state.data?.recording?.sessionId == recording.sessionId)
    ...
    private fun notifyExported(path: Path) {
        val notification =
            NotificationGroupManager.getInstance()
                .getNotificationGroup(NOTIFICATION_GROUP)
                .createNotification(text("live.exported"), NotificationType.INFORMATION)
        if (RevealFileAction.isSupported()) {
            notification.addAction(
                NotificationAction.createSimpleExpiring(RevealFileAction.getActionName()) {
    ...
    fun selectSite(id: Int) {
        val panel = view
        if (panel == null) pendingSite = id else panel.selectSite(id)
    }
    ...
    companion object {
        internal const val NOTIFICATION_GROUP = "JewelTooling.Live"
        private const val POLL_MS = 500L
        private const val HEARTBEAT_MS = 2000L
        private val LOG = Logger.getInstance(LiveInspectionService::class.java)
    }
}
