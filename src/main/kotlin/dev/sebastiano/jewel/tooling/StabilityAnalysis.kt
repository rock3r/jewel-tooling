...
internal object StabilityAnalysis {
    ...
    private fun KaSession.classify(
        ...
        if (
            symbol.annotations.any { annotation ->
                annotation.classId
                    ...
                    ?.annotations
                    ?.any {
                        it.classId?.asFqNameString() == "androidx.compose.runtime.StableMarker"
                    } == true
            }
        )
        ...
        val source =
            symbol.psi as? KtClass ?: return assessment(Stability.UNKNOWN, "reason.external")
        ...
        val usageFile = usage.containingKtFile.virtualFile
        val libraryUsage =
            usageFile != null && ProjectFileIndex.getInstance(usage.project).isInLibrary(usageFile)
        val sourceModule = ModuleUtilCore.findModuleForPsiElement(source)
        if (
            !libraryUsage &&
                (sourceModule == null ||
                    sourceModule != ModuleUtilCore.findModuleForPsiElement(usage))
        ) {
        ...
        try {
            ...
            for (property in
                symbol.declaredMemberScope.callables.filterIsInstance<KaPropertySymbol>()) {
                ProgressManager.checkCanceled()
                ...
                if (!property.isVal)
                    return assessment(
                        ...
                        .copy(sourceTarget = budget.target(property.psi))
                val result =
                    classify(property.returnType, usage, actuals, visiting, depth + 1, budget)
    ...
    private fun KaSession.classifyBinary(
        ...
        if (symbol.isInner || source.isLocal)
            return assessment(Stability.UNKNOWN, "reason.captured")
        val file =
            source.containingKtFile.virtualFile
                ?: return assessment(Stability.UNKNOWN, "reason.external")
        val id = type.classId
        val internalName =
        ...
        if (metadata.unstableBase)
            return assessment(Stability.UNSTABLE, "reason.compilerUnstable")
                .copy(evidence = compiler)
        ...
        try {
            ...
            return StabilityAssessment(
                ...
                evidence.toSet(),
                reasonCode =
                    if (reasons.isEmpty()) "reason.compilerStable" else "reason.compilerArgument",
            )
        } finally {
            visiting.remove(symbol)
        }
