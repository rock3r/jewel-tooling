...
internal class McpConfigUpdate(
  ...
  fun commit(candidate: String): Boolean {
    ...
    if (before != null) {
      ...
      val backup = privateRoot.resolve(Discovery.digest(path.toString().toByteArray()) + ".backup")
  ...
  private fun verifyContents(expected: ByteArray?) {
    if (!expected.contentEquals(McpConfigFiles.read(path, limit))) throw McpSetupFailure("changed")
  }
