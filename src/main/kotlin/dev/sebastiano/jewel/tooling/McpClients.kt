...
internal data class McpClientLaunch(
    ...
    companion object {
        ...
        fun create(bootstrap: Path, discovery: Discovery, root: Path): McpClientLaunch {
            ...
            val profile =
                Discovery.digest(bootstrap.parent.toRealPath().toString().toByteArray())
                    .take(ID_LENGTH)
...
internal class McpClientPaths(
    ...
    fun destination(client: McpClient): Path? =
        when (client) {
            ...
            McpClient.PI ->
                env("PI_CODING_AGENT_DIR", home.resolve(".pi/agent")).resolve("mcp.json")
    ...
    private fun studio(): Path? {
        ...
        return if (!Files.isDirectory(google)) null
            ...
            Files.list(google).use { dirs ->
                dirs
                    .filter {
                        it.fileName.toString().startsWith("AndroidStudio") && Files.isDirectory(it)
                    }
                    .limit(2)
                    .toList()
                    .singleOrNull()
