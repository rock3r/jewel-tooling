  ...
  fun install(
    ...
    McpConfigUpdate(registration, directory).use { record ->
      ...
      McpConfigUpdate(destination, directory, limit).use { config ->
        ...
        return config.commit(if (previous == entry) config.text else document.put(path, entry))
      }
    }
  }
    ...
    return Files.list(directory).use { files ->
      files
        ...
        .mapNotNull { file ->
          ...
          val record = McpJsonConfig(McpConfigFiles.text(McpConfigFiles.read(file))).objectValue()
          if (
