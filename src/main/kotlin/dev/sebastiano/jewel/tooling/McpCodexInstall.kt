...
internal class McpCodexInstall(private val process: McpProcessRunner = McpClientProcess()) {
    suspend fun install(
        ...
        McpConfigUpdate(destination, privateRoot).use { config ->
            ...
            try {
                ...
                val old = before.firstOrNull {
                    it.asJsonObject.get("name")?.asString == launch.name
                }
                if (old != null) {
                ...
                val gson = Gson()
                val command = gson.toJson(launch.command.first())
                val args = gson.toJson(launch.command.drop(1))
                ...
                // The CLI owns this isolated staging directory; it contains no real client
                // credentials.
    ...
    private suspend fun validateCandidate(
        ...
        val added =
            list(executable, stage).firstOrNull {
                it.asJsonObject.get("name")?.asString == launch.name
            }
