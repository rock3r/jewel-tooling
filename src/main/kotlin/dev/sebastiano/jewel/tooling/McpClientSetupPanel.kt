...
internal class McpClientSetupPanel(
  ...
  init {
    ...
    if (controller.skills.state.value.destination.isEmpty()) controller.skills.select(state.client)
    if (state.phase == "idle" && state.destination.isEmpty() && state.issue == null)
      controller.select(state.client)
  }

  private fun wrapped(text: String): String = "<html><body style='width: 380px'>$text</body></html>"
    ...
    result.text =
      wrapped(
        JewelToolingBundle.message(
          state.issue?.let { "mcp.install.error.$it" } ?: "mcp.install.state.${state.phase}"
        )
      )
    result.icon =
      when {
        state.issue != null -> AllIcons.General.Warning
        state.phase == "installed" || state.phase == "unchanged" -> AllIcons.General.InspectionsOK
        else -> null
      }
  }
}
