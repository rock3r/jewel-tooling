...
internal class RecordingSiteDetailsPanel(
  ...
  private fun bind(site: SiteSummary, labels: Map<Long, String>) {
    ...
    rows.add(
      section(editorText(declaration).apply { font = editorFont().deriveFont(Font.BOLD) }, 0, 8)
    )
    if (location != null) {
  ...
  private fun addMetrics(site: SiteSummary) {
    ...
    rows.add(
      section(
        secondary(JewelToolingBundle.message("recording.details.session", site.site.id.toString())),
        0,
        2,
      )
    )
    rows.add(
      section(
        secondary(JewelToolingBundle.message("recording.details.key", site.site.key.toString())),
        0,
        14,
      )
    )
  ...
  private fun addThreads(site: SiteSummary, labels: Map<Long, String>) {
    ...
    if (site.threads.isEmpty()) {
      rows.add(section(secondary(JewelToolingBundle.message("recording.threads.empty")), 0, 0))
      return
    }
        ...
        section(
          metric(
            JewelToolingBundle.message(
              ...
              labels[id].orEmpty().ifBlank { JewelToolingBundle.message("recording.unknown") },
              id.toString(),
            ),
