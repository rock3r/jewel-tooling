...
internal class McpAnalysisFacade(
  ...
  suspend fun inspect(tool: String, arguments: JsonObject): McpSnapshot {
    ...
    val snapshot = readAction {
      ...
      val psi = manager.getPsiFile(document) as? KtFile ?: throw McpFailure("UNSUPPORTED_FILE")
  ...
  private suspend fun loadDocument(path: Path): Document {
    val file =
      LocalFileSystem.getInstance().findFileByNioFile(path) ?: throw McpFailure("UNSUPPORTED_FILE")
    return readAction {
      ready()
      ...
      val document =
        FileDocumentManager.getInstance().getDocument(file) ?: throw McpFailure("UNSUPPORTED_FILE")
  ...
  private fun resolveFile(requested: String): Path {
    ...
    McpSourceInput.require(
      !relative.isAbsolute && relative.none { it.toString() == ".." } && requested.endsWith(".kt"),
      "UNSUPPORTED_FILE",
    )
...
private class McpDeclarationBuilder(
  ...
  private fun parameter(
    ...
    ProgressManager.checkCanceled()
    val receiver = function.receiverTypeReference?.takeIf { it.textRange.endOffset == hint.offset }
    val source =
      receiver
    ...
    return McpParameter(
      ...
      listOf(McpReason(assessment.reasonCode, assessment.reason, evidenceLocations(assessment))),
  ...
  private fun evidenceLocations(assessment: StabilityAssessment): List<McpLocation> {
    ...
    return if (
      targetFile != null && (index.isInContent(targetFile) || index.isInLibrarySource(targetFile))
    ) {
      val targetDocument =
