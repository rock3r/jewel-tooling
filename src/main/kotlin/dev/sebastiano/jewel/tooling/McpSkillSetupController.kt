...
internal class McpSkillSetupController(
  ...
  fun select(client: McpClient) {
    run(false) {
      McpSkillPaths(Path.of(System.getProperty("user.home")), EnvironmentUtil.getEnvironmentMap())
        .destination(client)
    }
  }
    ...
    job =
      scope.launch(io) {
        try {
          ...
          val installer =
            McpSkillInstaller(Path.of(PathManager.getConfigPath()).resolve("jewel-tooling-skills"))
          publish(selected) {
            it.copy(destination = path.toString(), bundledVersion = installer.bundle.version.text)
          }
          ...
          publish(selected) {
            it.copy(phase = "error", issue = (failure as? McpSetupFailure)?.reason ?: "failed")
          }
        } finally {
          publish(selected) {
            if (it.phase in setOf("checking", "installing")) it.copy(phase = "cancelled") else it
          }
        }
      }
  }
