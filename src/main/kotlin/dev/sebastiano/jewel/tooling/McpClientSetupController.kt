...
internal class McpClientSetupController(
  ...
  fun select(client: McpClient) {
    ...
    scope.launch(io) {
      try {
        ...
        val executable =
          command?.let { PathEnvironmentVariableUtil.findInPath(it)?.absolutePath }.orEmpty()
  ...
  fun install(request: McpInstallRequest) {
    ...
    mutableState.value =
      McpClientSetupState(request.client, request.destination, request.executable, "installing")
    job =
      scope.launch(io) {
        try {
  ...
  private fun failureReason(failure: Exception) = (failure as? McpSetupFailure)?.reason ?: "failed"
}
