...
internal class McpClientInstaller(
  ...
  suspend fun install(request: McpInstallRequest, launch: McpClientLaunch): Boolean {
    ...
    return when (request.client) {
      McpClient.CODEX ->
        McpCodexInstall(process).install(destination, request.executable, launch, privateRoot)
  ...
  private suspend fun configurePi(
    ...
    if (!before.contentEquals(McpConfigFiles.read(destination))) throw McpSetupFailure("piPartial")
  ...
  private suspend fun installPi(executable: String, agentDirectory: Path) {
    ...
    val config = McpJsonConfig(McpConfigFiles.text(McpConfigFiles.read(settings)).ifBlank { "{}" })
