...
class McpAnalysisTest : LightJavaCodeInsightFixtureTestCase() {
    ...
    fun testParityEvidenceAndUnsavedDocument() {
        ...
        val stale =
            JsonObject().apply { addProperty("declarationId", before.declarations.single().id) }
            ...
            runCatching {
                runBlocking {
                    ...
                    McpAnalysisFacade(project, "project", "generation", root) { true }
                        .inspect(
                            "jewel_analyze",
                            JsonObject().apply { addProperty("file", "Screen.kt") },
                        )
                }
            }
                .exceptionOrNull()
    ...
    fun testRangeValidationAndSourceLimit() {
        ...
        assertEquals(
            "INVALID_ARGUMENT",
            failure { call("jewel_analyze", range(extra, point)) }.code,
        )
        val outside = """{"line":200,"column":1}"""
        assertEquals(
            "STALE_LOCATION",
            failure { call("jewel_analyze", range(outside, outside)) }.code,
        )
        WriteCommandAction.runWriteCommandAction(project) {
        ...
        java.net.URLClassLoader(urls, ClassLoader.getPlatformClassLoader()).use { loader ->
            ...
            val errors =
                schema.javaClass
                    .getMethod(
                        "validate",
                        loader.loadClass("com.fasterxml.jackson.databind.JsonNode"),
                    )
    ...
    private fun call(
        ...
        return background {
            runBlocking {
                McpAnalysisFacade(project, "project", "generation", root) { enabled }
                    .inspect(tool, args)
            }
        }
    }
