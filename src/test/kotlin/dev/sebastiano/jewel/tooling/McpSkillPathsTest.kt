...
class McpSkillPathsTest : TestCase() {
  fun testAllClientDefaults() {
    ...
    McpClient.entries.zip(expected).forEach { (client, prefix) ->
      assertEquals(home.resolve("$prefix/${McpSkillBundle.ID}/SKILL.md"), paths.destination(client))
    }
  }
  ...
  fun testSkillOverridesAreIndependentOfMcpConfigurationOverrides() {
    ...
    assertEquals(custom.resolve("opencode/skills/$suffix"), paths.destination(McpClient.OPENCODE))
    assertEquals(home.resolve(".agents/skills/$suffix"), paths.destination(McpClient.CODEX))
    assertEquals(home.resolve(".config/agents/skills/$suffix"), paths.destination(McpClient.AMP))
    assertEquals(
