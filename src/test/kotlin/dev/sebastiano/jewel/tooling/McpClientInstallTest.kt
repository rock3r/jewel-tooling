...
class McpClientInstallTest : TestCase() {
    ...
    fun testJsonClientsPreserveSettingsAndAreIdempotent() = runBlocking {
        ...
        for (client in clients) {
            ...
            assertEquals(
                "keep",
                parsed.value(listOf(client.container, "other", "command"))!!.asString,
            )
    ...
    fun testJsonParserRejectsDuplicatesMalformedAndDeepInput() {
        ...
        assertEquals(
            "value",
            McpJsonConfig(
                    document.put(listOf("nested", "key"), com.google.gson.JsonPrimitive("value"))
                )
    ...
    fun testConflictingEntryAndConcurrentChangesRemainUntouched() {
        ...
        Files.writeString(
            file,
            """{"mcpServers":{"jewel-profile-project":{"command":"foreign"}}}""",
        )
    ...
    fun testLargeClaudeConfigAndCanonicalOwnership() {
        ...
        val reordered =
            JsonObject().apply { entry.keySet().reversed().forEach { add(it, entry[it]) } }
        ...
        assertTrue(
            owned.install(McpClient.CLAUDE, file, repaired, McpClient.CLAUDE.entry(repaired))
        )
    }
    ...
    fun testDifferentProfilesAndProjectsKeepSeparateEntries() {
        ...
        for (name in
            listOf(
                "jewel-profile1-project1",
                "jewel-profile1-project2",
                "jewel-profile2-project1",
            )) {
    ...
    fun testStudioRefreshRevocationAndForeignEdit() = runBlocking {
        ...
        try {
            ...
            val target =
                McpClientLaunch.create(privateFiles.resolve("bootstrap.jar"), discovery, project)
            ...
            discovery.publish(
                mapOf("endpoint" to "http://127.0.0.1:43211/mcp", "token" to "second")
            )
    ...
    fun testPathsAndAmbiguousStudioProfiles() {
        ...
        Files.createDirectories(
            temporary.resolve("Library/Application Support/Google/AndroidStudio1")
        )
        assertNotNull(paths.destination(McpClient.ANDROID_STUDIO))
        Files.createDirectories(
            temporary.resolve("Library/Application Support/Google/AndroidStudio2")
        )
        assertNull(paths.destination(McpClient.ANDROID_STUDIO))
        val windows =
            McpClientPaths(
